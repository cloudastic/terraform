...
module "network" {
  ...
  subnets = {
    prod_snet_1 = { address_prefixes = ["10.0.10.0/24"] } # Create new Subnet for each VM modules. 
    prod_snet_2 = { address_prefixes = ["10.0.20.0/24"] } # Create new Subnet for each VM modules. 
  }
}
...
module "vm-test-1-module" {
  ...
  virtualmachines_subnet = module.network.subnet_ids["prod_snet_1"]
...
module "vm-test-2-module" { # UPDATE HERE : Change the module name to be in-line with the VM created
  ...
  location               = "northeurope"
  virtualmachines_rg     = module.resource_group
  virtualmachines_subnet = module.network.subnet_ids["prod_snet_2"] # UPDATE HERE : Create a new subnet for each VM and associate the same here
  nsg_id                 = module.network.nsg_id
  vm_size                = "Standard_D2s_v3"
  disk_size_gb           = 200
  admin_username         = "Elekta"
